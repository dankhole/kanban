	...
	return {
		...
		async getProviderCatalog(): Promise<RuntimeClineProviderCatalogResponse> {
			...
			const providers: RuntimeClineProviderCatalogItem[] = await listSdkProviderCatalog()
				.then((sdkProviders) =>
					sdkProviders.map((provider) => ({
						id: provider.id,
						name: provider.name,
						oauthSupported: (provider.capabilities ?? []).includes("oauth"),
						enabled: selectedProviderId.length > 0 ? selectedProviderId === provider.id : provider.id === "cline",
			...
			if (!providers.some((p) => p.id === "openai-compatible")) {
				providers.push({
					...
					enabled: selectedProviderId.length > 0 ? selectedProviderId === "openai-compatible" : false,
					defaultModelId: null,
				});
			}
